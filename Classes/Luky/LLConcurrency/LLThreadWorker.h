...
The LLThreadWorker class was designed to be simple and
...
Be sure to copy the LLThreadWorker.h and LLThreadWorker.m
...
SourceForge contact you when new versions of LLThreadWorker
are released:
...
0.7 -
 ...
 o  Changed the behavior when "longTask" takes a second argument.
    ...
    it passes a references to the LLThreadWorker. The recommended way
...
LLThreadWorker* to allow for passing of the parent LLThreadWorker
...
*/
@interface LLThreadWorker : LLObject
{
...
@result Returns an autoreleased LLThreadWorker that manages the worker thread.
...
which will be the controlling LLThreadWorker instance which you can use
to see if the LLThreadWorker has been marked as cancelled.
Your "longTask" method might then look like this:
 <pre>
    - (id)longTask:(id)userInfo anyNameHere:(LLThreadWorker *)tw
   {
       ...
       while(... && ![tw cancelled]){
           ...
...
match it when you create the LLThreadWorker like so:
 <pre>
    [LLThreadWorker workOn:self 
...
Of course you will have to have imported the LLThreadWorker.h
...
+ (LLThreadWorker *)
    ...
    /*!
     ...
     @abstract Mark the LLThreadWorker as cancelled.
     @discussion
     Marks the LLThreadWorker as cancelled but doesn't actually
     cancel the thread. It is up to you to check whether or
     not the LLThreadWorker is cancelled using a two-argument
